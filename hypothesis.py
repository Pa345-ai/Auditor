# core/hypothesis.py
"""
Vulnerability Hypothesis Engine – with false positive reduction and user-resource IDOR detection.

Changes:
  - Public endpoint allowlist (skip MissingAuthentication on known public writes).
  - IDOR generation only if response model has ownership fields OR is a user resource model.
  - Path asymmetry only when unauthenticated method is write and authenticated methods are reads.
  - Extended ID parameter patterns to include "username", "user" for better coverage.
"""
...
class HypothesisEngine:
    ...
    # ── Helper: Identify user-resource models ─────────────────────────────────
    def _is_user_resource_model(self, model_node: ModelNode) -> bool:
        """
        Returns True if the model represents a user (or other sensitive individual).
        Heuristics: model name contains 'User', or fields include typical user fields.
        """
        name_lower = model_node.label.lower()
        if "user" in name_lower:
            return True
        # Check fields for user-like attributes
        user_fields = {"username", "email", "password", "phone", "address", "name", "contact"}
        for field_id in model_node.fields:
            field = self.graph.get_node(field_id)
            if isinstance(field, ModelFieldNode) and field.label.lower() in user_fields:
                return True
        return False

    # ── IDOR (corrected with ownership field check + user-resource detection) ─
    def _check_idor(self) -> None:
        """
        Pattern: endpoint accepts a path/query ID param + endpoint is authenticated
        + the model the ID references has an owner field OR is a user resource
        ...
        for ep in self.graph.endpoints():
            ...
            # Find ID-looking path/query parameters (extended patterns)
            id_params = self._id_params_for_endpoint(ep)
            if not id_params:
                continue

            # Check if any response model has ownership fields or is a user resource
            ...
            for edge in self.graph.outgoing(ep.id):
                if edge.type == EdgeType.RESPONSE_SHAPE:
                    ...
                    if isinstance(model, ModelNode):
                        if model.ownership_fields:
                            has_ownership_model = True
                        if self._is_user_resource_model(model):
                            is_user_resource = True
                    if has_ownership_model or is_user_resource:
                        break

            # If the resource has no ownership fields and is not a user resource, IDOR is impossible
            if not (has_ownership_model or is_user_resource):
                continue
            ...
            for param in id_params:
                evidence = [
                    ...
                    f"Response model is user resource: {is_user_resource}",
                ]
                ...
                if has_ownership_constraint:
                    ...
                    conf = 0.82 if has_ownership_model else 0.68
                    desc = (
                        f"IDOR: {ep.method.value} {ep.path} accepts user-controlled "
                        f"'{param.label}' with no ownership enforcement detected, "
                        f"and the resource is {'user-owned' if has_ownership_model else 'a user resource'}."
                    )
                    severity = "High"
    ...
    def _id_params_for_endpoint(self, ep: EndpointNode) -> list[ParameterNode]:
        """Return params that look like object ID selectors (extended for usernames)."""
        id_patterns = ["_id", "id", "Id", "ID", "uuid", "key", "token", "username", "user"]
        result = []
    ...
    def _check_missing_ownership_on_related_model(self) -> None:
        ...
        OR is a user resource model, but no auth context constrains by those fields.
        ...
        for ep in self.graph.endpoints():
            ...
            for edge in self.graph.outgoing(ep.id):
                ...
                # NEW: Check if this is a user resource model even without explicit ownership fields
                is_user_resource = self._is_user_resource_model(model)

                if model.ownership_fields or is_user_resource:
                    has_id_param = bool(self._id_params_for_endpoint(ep))
                    ...
                    h = AttackHypothesis(
                        ...
                        description=(
                            ...
                            f"which {'has ownership fields ' + str(model.ownership_fields) if model.ownership_fields else 'is a user resource model'}, "
                            f"but no ownership constraint enforced in auth."
                        ),
                        chain=[ep.id, model.id],
                        severity_estimate="High",
                        confidence=0.75 if model.ownership_fields else 0.68,
                        evidence=[
                            f"Model '{model.label}' is user resource: {is_user_resource}",
                            f"Ownership fields present: {model.ownership_fields}",
                            f"Endpoint has no ownership constraint",
                            f"Accepts user-controlled ID: {has_id_param}",
                        ],
                        attack_transform=(
                    ...
                    self.hypotheses.append(h)
                    break  # one hypothesis per endpoint is enough
